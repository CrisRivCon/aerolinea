<x-app-layout>
    <div class="w-1/2 mx-auto">
        <form method="POST" enctype="multipart/form-data" action="{{ route('vuelos.update', ['vuelo' => $vuelo]) }}">
            @csrf
            @method('PUT')
            <!-- Nombre -->
            <div>
            ...
            <div class="mt-4">
                ...
                <x-text-input id="plazas" class="block mt-1 w-full" type="text" name="plazas" :value="old('plazas', $vuelo->plazas)"
            ...
            <div class="mt-4">
                ...
                <x-text-input id="precio" class="block mt-1 w-full" type="text" name="precio" :value="old('precio', $vuelo->precio)"
            ...
            <div class="mt-4">
                ...
                <select id="companya_id"
                    ...
                    @foreach ($companyas as $companya)
                        <option value="{{ $companya->id }}" {{ old('companya_id', $vuelo->companya) == $companya->id ? 'selected' : '' }}>
                            {{ $companya->nombre }}
                        </option>
                    @endforeach
                </select>
                ...
                <select id="origen_id"
                    ...
                    @foreach ($aeropuertos as $aeropuerto)
                        <option value="{{ $aeropuerto->id }}"
                            {{ old('origen_id', $vuelo->origen_id) == $aeropuerto->id ? 'selected' : '' }}>
                            {{ $aeropuerto->codigo }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mt-4">
                ...
                <select id="destino_id"
                    ...
                    @foreach ($aeropuertos as $aeropuerto)
                        <option value="{{ $aeropuerto->id }}"
                            {{ old('destino_id', $vuelo->destino_id) == $aeropuerto->id ? 'selected' : '' }}>
                            {{ $aeropuerto->codigo }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mt-4 flex">
                ...
                <div class="mt-4">
                    ...
                    <input type="file" name="imagen" id="imagen" value="{{ old('imagen', asset('storage/uploads'.$vuelo->imagen)) }}">
